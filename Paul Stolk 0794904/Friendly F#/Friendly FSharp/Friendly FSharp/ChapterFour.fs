namespace ChapterFour
 module LargeAsteroidFieldSimulation =
  ...
  type Asteroid3D = 
    {
      ...
      Velocity: Vector3<meters/seconds>
      Mass: float<kg>
      Name: string
    }
  ...
  let create_field num_asteroids: Asteroid3D list =
    ...
    [
      for i = 1 to num_asteroids do
        ...
        let z = lerp 0.0<meters> field_size (rand.NextDouble())
  ...
  let fast_simulation_step (asteroids:Asteroid3D list) =
    let empty_tree =
      OctaTree<_,_,_>.mk_empty
        { Min = Vector3<_>.Zero;
          Size = { X = field_size/8.0; Y = field_size/8.0; Z = field_size/8.0 } }
        { Min = Vector3<_>.Zero;
          Size = { X = field_size; Y = field_size; Z = field_size } }
    let tree =
      List.fold (fun t a ->
        OctaTree<_,_,_>.insert (fun (a: Asteroid3D) -> a.Position) a t)
        empty_tree asteroids
    let tree =
      OctaTree<_,_,_>.fold (fun a b c d e f g h-> ((a +/ b) +/ (c +/ d)) +/ ((e +/ f) +/ (g +/ h))) 
    ...
    let rec traverse (others: Barycenter) =
      function
      | OctaTree.Leaf(r, a, b) -> OctaTree.Leaf(r, local_forces others a, b)
      | OctaTree.Node(r, q111, q112, q121, q122, q211, q212, q221, q222, b) ->
        let q111' =
  ...
  let print_scene (asteroids: Asteroid3D list) =
    ...
    for a in asteroids do
      ...
      let z = ((a.Position.Z / 4.0e8<meters>) * 8.0 + 1.0) |> int
      do Console.Write(z)
    do Thread.Sleep(10)
